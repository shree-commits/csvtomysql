...
class CSVToMySQLImporter:
    ...
    def import_csv(self, filename, delimiter, columns, table):
        """Import CSV file to MySQL database, with error handling for data discrepancies."""
        error_log_file = "error_log.txt"

        try:
            with open(filename, 'r', newline='') as csv_file, open(error_log_file, 'w') as error_log:
                ...
                for row_number, row in enumerate(csv_reader, start=1):
                    try:
                        ...
                        if len(row) != len(column_headers):
                            raise ValueError(f"Row {row_number} has {len(row)} columns, expected {len(column_headers)}.")
                        ...
                        error_message = f"Error in row {row_number}: {e}, Data: {row}\n"
                        error_log.write(error_message)
                        print(f"Logged error for row {row_number}. Continuing...")
                        continue

                self.db.commit()

                end_time = time.time()
                print("\nData import successful with errors logged to 'error_log.txt' (if any).")
